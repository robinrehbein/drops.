...
(An earlier draft also guarded `raw < 0`, but with the prior guards on `doseG > 0`, `yieldG > 0`, and a non-negative default `tdsAssumed`, that branch is unreachable for any documented input. Dropping it keeps the 100% coverage gate green.)
...
describe('schema', () => {
  ...
  it('soft-delete sets deleted_at and live queries filter it', () => {
    // Insert one live row + one soft-deleted row, query for live rows via
    // isNull(beans.deletedAt), and assert exactly one returned. (Using
    // `eq(col, null)` is vacuous SQL — `col = NULL` is never TRUE — so we
    // use `isNull` from 'drizzle-orm' to express "IS NULL" properly.)
    const db = makeTestDb();
    const liveId = uuid();
