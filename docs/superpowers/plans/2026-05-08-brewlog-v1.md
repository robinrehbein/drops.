...
```bash
npm i -D jest jest-expo @types/jest @testing-library/react-native ts-jest
```

(Note: `@testing-library/jest-native` is **not** installed — it's been deprecated since RNTL 12.4 in favor of built-in matchers.)
...
- [ ] **Step 5: Sanity test — run jest with no tests**
