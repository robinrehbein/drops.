# Drop v1 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship Drop v1 to App Store and Play Store — Brew Lab + Bean Library + thin Daily Brew dashboard, espresso-only, local-first.
...
```bash
cd /Users/robin.rehbein/Code/drop
...
```json
{
  "expo": {
    ...
    "newArchEnabled": true,
    "ios": { "bundleIdentifier": "com.drop.app", "supportsTablet": false },
    "android": { "package": "com.drop.app" },
    "plugins": ["expo-router"],
...
Expected: simulator opens, blank screen with "Drop" centered. Stop the dev server with `q`.
...
export default function RootLayout() {
  ...
  if (error) {
    return (
      <ThemeProvider>
        <View style={{ flex: 1, padding: 24, alignItems: 'center', justifyContent: 'center' }}>
          <Text variant="title" align="center">Drop needs to rebuild its data.</Text>
...
export default function LabIndex() {
  ...
  if (status === 'Pulling') useKeepAwake('drop-pulling');
  ...
  return (
    <Sheet>
      ...
      <ScrollView contentContainerStyle={{ gap: t.space.md, marginTop: t.space.lg }}>
        ...
        <Row label="About" value={`Drop v${require('../../app.json').expo.version}`} />
      </ScrollView>
...
See `docs/superpowers/specs/2026-05-08-drop-design.md` for the v1 design spec.
```
...
**Plan complete and saved to `docs/superpowers/plans/2026-05-08-drop-v1.md`. Two execution options:**
