/* Validates a raw research export and writes the bundled seed JSON.
   Usage: node scripts/build-places-seed.js <input.json> [--curated]
   Input may be either { places: [...] } or a bare array.
   --curated keeps only editorially-confirmed specialty places (curated:true). */
const fs = require('fs');
const path = require('path');

const args = process.argv.slice(2).filter((a) => !a.startsWith('--'));
const curatedOnly = process.argv.includes('--curated');
const inputArg = args[0] || '/tmp/germany-specialty-coffee.json';
...
console.log(
  `Wrote ${seed.length} places (${out.curatedCount} curated${curatedOnly ? ', curated-only' : ''}) -> ${dest}`,
);
